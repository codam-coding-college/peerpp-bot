...
export async function listProjectIds(say: SayFn) {
	...
	for (const project of env.projects) text += `- \`${project.slug}\`\n`;
	await say(text);
}
...
function countProjects(locks: Intra.ScaleTeam[]) {
	...
	for (const lock of locks) {
		...
		if (
			...
			count[lock.projectSlug]!.createdAt.getTime()
		)
...
export async function listEvaluations(say: SayFn) {
	await say(
		"Getting evaluation locks, this can take more than 10 seconds..."
	);
	...
	const longestName: number = Math.max(
		...locks.map((lock: Intra.ScaleTeam) => lock.projectSlug.length)
	);

	let text = "Peer++ evaluations, highest priority first\n";
	text +=
		"Format: <project_name> <number_of_evaluations> <time_since_lock>\n";
	text += "```\n";
	...
	for (const key in count) {
		...
		const timeLocked = prettyMilliseconds(
			Date.now() - count[key]!.createdAt.getTime(),
...
export async function bookEvaluation( say: SayFn, corrector: User, projectSlug: string): Promise<void> {
	await say(
		`Requested peer++ eval by ${corrector.intraLogin} for \`${projectSlug}\`...`
	);
	...
	await log(
		`Booked evaluation corrector: ${corrector.intraLogin}, correcteds ${lock.correcteds} on ${projectSlug}`
	);
	...
	for (const user of correcteds) text += `@${user.intraLogin} `;
